...
def seidel(aelem, jptr, iptr, eps):
    ...
    while not converge:
        x_new = deepcopy(x)
        for i in range(n):
            s1 = sum(matrix(aelem, jptr, iptr,i, j) * x_new[j] for j in range(i))
            s2 = sum(matrix(aelem, jptr, iptr,i, j) * x[j] for j in range(i + 1, n))
            x_new[i] = (matrix(aelem, jptr, iptr, i, n) - s1 - s2) / matrix(aelem, jptr, iptr,i, i)
